...
import json
from .models import smartpipe,pipedetail,project
...
# my_conn_s =
config = {
    'host':'rm-uf6k42ke05jq7358so.mysql.rds.aliyuncs.com',
    'port':3306,
    'user':'root_top',
    'password':'root_Top',
    'db':'smartpipe',
...
def index(request):
    ...
    c_longitude = project_info[0].longitude
    c_latitude = project_info[0].latitude

    for i in range(len(project_info)):
        p_name.append(project_info[i].name)
        p_address.append(project_info[i].address)
        p_longitude.append(project_info[i].longitude)
        p_latitude.append(project_info[i].latitude)

    return render(request, 'smartpipe/map_address.html',
                  {'address_longitude': json.dumps(address_longitude),#经度，标记点
                   'address_latitude': json.dumps(address_latitude),#纬度
                   'address_data': json.dumps(address_data),
                   'address_id': json.dumps(address_id),
                   'p_name': json.dumps(p_name),
                   'p_address': json.dumps(p_address),
                   'p_longitude': json.dumps(p_longitude),
                   'p_latitude': json.dumps(p_latitude),#中心点
...
#
#     for i in range(len(project_info)):
#         name.append(project_info[i].name)
#         user.append(project_info[i].user)
#         address.append(project_info[i].address)
#         longitude.append(project_info[i].longitude)
#         latitude.append(project_info[i].latitude)
#
#     return render(request, 'smartpipe/map_address.html',
...
class pipeS(View):
    def get(self, request, meter_id):
        ...
        def line_ylll() -> Grid:
            bar = (
                Bar(init_opts=opts.InitOpts(width=w1, height=h1))
                    .add_xaxis(l_a)
                    .add_yaxis("流量(吨)", l_v2)
            ...
            line = (
                Line()
                    .add_xaxis(l_a)
                    .add_yaxis("压力（Mba）", l_v1, is_smooth=True)
                    .set_series_opts(label_opts=opts.LabelOpts(is_show=False, font_size=10),
                                     )
                    .set_global_opts(
            ...
            bar.overlap(line)
            return Grid().add(bar, opts.GridOpts(pos_left="5%", pos_right="20%", width=450, height=80),
        ...
        context = dict(
            ...
            t_tem=s_tem,
            t_power=s_power,
            t_chart_yl=line_ylll().render_embed(),
        )

        template = loader.get_template('smartpipe/collectinfoS.html')
        ...
        def line_ylll() -> Grid:
            bar = (
                Bar()
                    ...
                    .set_global_opts(
                    title_opts=opts.TitleOpts(title=f" [ {s_name}]最近30"),
                    xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(rotate=30), ),  # 设置x轴标签旋转角度
                )
            ...
            bar.overlap(line)
            return Grid().add(bar, opts.GridOpts(pos_left="5%", pos_right="20%", width=500, height=80),
                              is_control_axis_index=True)
        # bar_lc1().render_notebook()
