...
class OrgView(View):
    def get(self,request):
        ...
        if tag:
            # 我们就在类型中作进一步筛选类别
            all_orgs = all_orgs.filter(tag=tag)
    ...
    def get(self, request, org_id):
        ...
        if request.user.is_authenticated:
            if UserFavorite.objects.filter(user=request.user, fav_id=course_org.id, fav_type=2):
                has_fav = True
        # 通过课程类型找到课程。内建的变量，找到指向这个字段的外键引用
...
class TeacherCourseView(View):
   ...
   专家课程列表页
    ...
    def get(self, request, teacher_id):
        ...
        current_page = "course"
        # 根据id取到课程类型
        course_teacher = Teacher.objects.get(id= int(teacher_id))
        # 通过专家类型找到课程。内建的变量，找到指向这个字段的外键引用
        all_courses = course_teacher.course_set.all()
        # 向前端传值说明用户是否收藏
        has_fav = False
        # 必须是用户已登录我们才需要判断。
        if request.user.is_authenticated:
            if UserFavorite.objects.filter(user=request.user, fav_id=course_teacher.id, fav_type=2):
                has_fav = True
        return render(request, 'teachers-list.html',{
           'all_courses':all_courses,
