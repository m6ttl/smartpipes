    ...
    def post(self, request, *args, **kwargs):
        ...
        if text.startswith('@['):
            ...
            try:
                ...
                info = u'{}回复了你在 {} 的交流'.format(
                    user.username,
                    parent.article.title,
                )
             
            except Comment.DoesNotExist:
                logger.error(u'[CommentView]交流引用错误:%s' % parent_str)
                return HttpResponse(u'请勿修改交流代码！', status=403)

        if not text:
            logger.error(u'[CommentView]当前用户输入空交流：[{}]'.format(user.username))
            return HttpResponse(u'请输入交流内容！', status=403)
        ...
        print_comment = u'<p>交流：{}</p>'.format(text)
        if parent:
